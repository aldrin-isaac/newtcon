...
// Duplicated from server.newUUID to avoid the circular dependency
// handlers → server → types (server already imports types).
// Both use crypto/rand and produce the same xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx format.
func genUUID() string {
	var b [16]byte
