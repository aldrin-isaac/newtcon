...
web/                          → frontend (vanilla HTML + TypeScript-as-tsc per ADR-0002)
  src/                        → source
    ...
    device-resources.ts       → deviceServiceUsage + countServiceInstances — pure resource lens (inverse of the interface table): groups a topology device's apply-service steps by service → the interfaces each is applied to (+ per-instance vlan/ip/peer-AS); backs the device drawer's State-tab "Services" section
