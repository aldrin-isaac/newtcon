...
const BASE=process.env.NEWTCON_URL||"http://127.0.0.1:8095", NET="1node-vs";
...
try{
  ...
  await p.click('[data-kind="qos-policies"]'); await p.waitForSelector(".panel-add-btn",{timeout:20000});
  ...
  await p.waitForSelector(".apply-preview-card",{timeout:20000}); await sleep(300);
  ...
  // results modal
  await p.waitForFunction(()=>{const t=document.querySelector(".apply-preview-title")?.textContent||"";return /Applied 1 · Failed 1/.test(t);},{timeout:20000});
