...
//
// Matches on newtron's server-derived spec_kind/spec_name (#282), not a
// client-side name canonicalization. serviceName is the canonical name
// (the service detail's `.name`, same domain as spec_name).
  ...
  devices: {
    switch1: {
      steps: [
        ...
        { url: "/interfaces/Ethernet16/apply-service", spec_kind: "service", spec_name: "overlay-irb-a",
...
describe("deriveServiceBindings()", () => {
  test("finds every interface a service is applied to, across devices", () => {
    const b = deriveServiceBindings(TOPO, "overlay-irb-a");
    assert.equal(b.length, 2);
    assert.deepEqual(b.map((x) => `${x.device}:${x.iface}`), ["switch1:Ethernet16", "switch2:Ethernet16"]);
  });

  test("matches spec_name via canonicalization (authored list name ↔ canonical spec_name)", () => {
    // The drawer passes the authored list name; spec_name is canonical.
    // Both forms must resolve to the same bindings.
